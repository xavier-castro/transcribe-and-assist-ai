...
export default function Home() {
  ...
  const handleStartProcessing = async (videoUrl: string) => {
    ...
    if (typeof videoId === 'string') {
      ...
      const getTranscript = await processVideo(videoId, message => {
        setProgressOutput(prev => prev + message)
      })
      if (getTranscript) {
        setResultTranscript(getTranscript)
      }

      console.log('Transcript: ', getTranscript)
      setProcessing(false)
      setActiveTab('result')
    } else {
      alert('Invalid URL')
  ...
  return (
    <Box css={{ paddingY: '$6' }}>
      <Head>
        <title>Transcribe and Assist Ai</title>
      </Head>
      <Container size={{ '@initial': '1', '@bp1': '2' }}>
        <Text as="h1">Transcribe and Assist Ai</Text>
        <VideoForm
